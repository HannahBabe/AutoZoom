...
import  datetime, time, subprocess, csv, os, webbrowser, psutil
...
for i in range(len(meetings)):
    ...
    while True:
        ...
        if imagesearch_region_loop('join.jpeg', 5, 912, 565, 1011, 610) != None :
            var = imagesearch_region_loop('join.jpeg', 5, 912, 565, 1011, 610)
            pyautogui.click(var)
        """
        if (time.time() - cur) >= ((curmeeting[4] * 60) + 120):
            pyautogui.click(x=1634, y=1021, interval=2)
            pyautogui.click(x=1565, y=956)
            break
        time.sleep(5)
...
"""
    ...
    if pyautogui.locateOnScreen('mute.png', confidence=0.9) != None :
        ...
        pyautogui.click(var)
                ...
                if pyautogui.locateOnScreen('leave.png', confidence=0.5) != None :
                var = pyautogui.locateOnScreen('leave.png', confidence=0.5)
                pyautogui.click(var)
                print("got here")
                if pyautogui.locateOnScreen('leavemeeting.png', confidence=0.5) != None :
                    var = pyautogui.locateOnScreen('leavemeeting.png', confidence=0.5)
                    pyautogui.click(var)
                    print("got here too")
"""
